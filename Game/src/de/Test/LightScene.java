...
public class LightScene extends Scene {
	...
	public void init() {
		...
		cube = new GameObject(
				OBJLoader.ModelFromFile("cube.obj", "bricks2.jpg"));
		// cube = new GameObject(OBJLoader.ModelFromFile("cube.obj",
		// "white.png"));
		cube.getTransform().setPosition(new Vector3f(5, 1, 10));
		cube.getTransform().setRotation(
				Quaternion.fromAxisAngle(Vector3f.left(), 0.8f));
		cube.getModel().getMaterial()
				.setNormalMap(Texture.loadFromFile("bricks2_normal.jpg", true));
		cube.getModel().getMaterial()
				.setDispMap(Texture.loadFromFile("bricks2_disp.jpg", true));
		...
		for (int i = 0; i < 10; i++) {
			...
			l.getTransform().setPosition(
					new Vector3f((float) Math.random() * 100 - 50, 0.5f,
